...
export default function Home() {
  ...
  async function handleSubmitToLeaderboard() {
    if (!result || leaderboardSubmitting || leaderboardSubmitted) return;
    setLeaderboardSubmitting(true);
    try {
      let thumbnailUrl: string | undefined;
      if (previewUrl) thumbnailUrl = (await makeThumbnail(previewUrl)) ?? undefined;
      ...
      const lbRes = await fetch("/api/leaderboard/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ resultId: saveData.id }),
      });
      if (lbRes.ok || lbRes.status === 409) setLeaderboardSubmitted(true);
    } catch {
      // silently fail
    } finally {
      setLeaderboardSubmitting(false);
